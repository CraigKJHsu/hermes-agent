...
class MemoryAdjudicator:
    ...
    def __init__(
        ...
        *,
        ...
        self.max_cache_entries = max(1, int(cfg.get("max_cache_entries", 1024)))
        ...
        if bullet_mode:
            for line in lines:
                ...
                if stripped.startswith("- "):
                    ...
                    if re.fullmatch(r"#{1,6}\s+\S.*|.{1,80}:", stripped):
                        header.append(line)
                    else:
    ...
    def _record_decision(
        ...
        *,
        ...
        if decision.decision == "ACCEPT" and not self.audit_accepts:
            return
        try:
            audit_parent_existed = self.audit_db.parent.exists()
            self.audit_db.parent.mkdir(parents=True, exist_ok=True, mode=0o700)
            if not audit_parent_existed:
                try:
    ...
    def _cache_put(self, key: str, decision: MemoryDecision) -> None:
        with self._cache_lock:
            ...
            if self.cache_ttl_seconds:
                expired = [
                    cache_key
                    for cache_key, (written_at, _decision) in self._cache.items()
