...
_TRAILING_TRANSLATION_DIRECTIVES = (
    ...
    (
        re.compile(
            r"(?<=[,，])\s*translate\s+(?:(?:it|this)\s+)?(?:into|to)\s+"
            + r"english" + _TRAILING_PUNCTUATION,
            re.IGNORECASE,
        ),
        "en",
...
_LEADING_TRANSLATION_DIRECTIVES = (
    (
        re.compile(
            r"^\s*(?:請|麻煩(?:你|妳)?|幫我)?\s*翻譯(?:成|為)?\s*"
            r"(?:英文|英語)\s*[:：]\s*(?P<source>.+)$",
    ...
    (
        re.compile(
            r"^\s*(?:please\s+)?translate\s+(?:(?:it|this)\s+)?"
            r"(?:into|to)\s+english\s*[:：]\s*(?P<source>.+)$",
            re.IGNORECASE | re.DOTALL,
        ),
        "en",
    ),
    (
        re.compile(
            r"^\s*(?:please\s+)?translate\s+(?:(?:it|this)\s+)?"
            r"(?:into|to)\s+(?:traditional\s+)?chinese\s*[:：]\s*"
            r"(?P<source>.+)$",
            re.IGNORECASE | re.DOTALL,
        ),
        "zh-TW",
...
def prepare_direct_translation_input(text: str) -> tuple[str, str]:
    ...
    for pattern, target in _LEADING_TRANSLATION_DIRECTIVES:
        leading = pattern.fullmatch(text)
        if leading is not None:
            source = leading.group("source").strip()
            if source:
