...
def normalize_response_profile(value: Any) -> Optional[Dict[str, Any]]:
    ...
    intent_gate = str(value.get("intent_gate") or "").strip().lower()
    if intent_gate not in {"", "translation_or_answer"}:
        return None
    ...
    normalized_profile = NormalizedResponseProfile({
    ...
    if intent_gate:
        normalized_profile["intent_gate"] = intent_gate
...
_TRANSLATION_INTENT_PATTERNS = (
    re.compile(
        r"^(?:(?:(?:請|麻煩|拜託)(?:你|妳)?(?:幫我)?|幫我)\s*)?"
        r"(?:翻譯成|翻成|譯成)[^\s:：，,。！？?]{1,20}"
        r"(?=[:：\s，,。！？?]|$)|"
        r"^翻譯(?:成(?:英文|英語|中文|華語|繁體中文|日文|日語|韓文|韓語|"
        r"法文|法語|德文|德語|西班牙文|西班牙語))?(?:[:：\s]|$)|"
        r"^(?:(?:請問|請|麻煩|拜託)(?:你|妳)?\s*|(?:你|妳)?(?:可以|能|"
        r"能否|能不能)(?:請你)?\s*|幫我\s*|(?:把|將)\s*)"
        r"(?:幫我\s*)?(?:(?:把|將)\s*)?(?:以下|這(?:個|句|段|篇)?|"
        r"那(?:個|句|段|篇)?)?\s*(?:文字|內容|單字|詞|片語|句子|"
        r"文章)?\s*(?:翻譯(?:成)?(?=(?:英文|英語|中文|華語|繁體中文|"
        r"日文|日語|韓文|韓語|法文|法語|德文|德語|西班牙文|"
        r"西班牙語|English|Chinese|Japanese|Korean|French|German|"
        r"Spanish|[:：\s]|這|那|以下|$))|翻成|譯成)"
        r"(?:英文|英語|中文|華語|繁體中文|日文|日語|韓文|韓語|"
        r"法文|法語|德文|德語|西班牙文|西班牙語|English\b|Chinese\b|"
        r"Japanese\b|Korean\b|French\b|German\b|Spanish\b)?|"
        r"(?:英文|英語|中文|華語|繁體中文|日文|日語|韓文|韓語|"
        r"法文|法語|德文|德語|西班牙文|西班牙語)\s*(?:要)?怎麼說|"
        r"怎麼用(?:英文|英語|中文|華語|繁體中文|日文|日語|韓文|韓語|"
        r"法文|法語|德文|德語|西班牙文|西班牙語)說|"
        r"^(?!(?:為什麼|如何|怎麼|請問|誰|哪(?:個|些|裡)|何時|何處))"
        r".{1,80}(?:是什麼意思|意思是什麼|怎麼念|如何發音|"
        r"怎麼發音|音標是什麼)|"
        r"^.{1,80}?的?(?:英文|中文|繁體中文)(?:翻譯)?(?:是|為)什麼|"
        r"^.{1,80}?的?(?:定義|意思)(?:是|為)什麼|"
        r"(?:英文|中文|繁體中文)翻譯(?:是|為)什麼",
        re.IGNORECASE,
    ),
    re.compile(
        r"^(?:please\s+)?(?:translate|pronounce|define)\b|"
        r"^(?:can|could|would|will)\s+you\s+(?:please\s+)?"
        r"(?:translate|pronounce|define)\b|"
        r"^(?:can|could|would|will)\s+you\s+(?:please\s+)?"
        r"tell\s+me\s+what\b.{1,120}\bmeans\b|"
        r"^(?:would|could)\s+you\s+mind\s+"
        r"(?:translat(?:e|ing)|pronounc(?:e|ing)|defin(?:e|ing))\b|"
        r"^how\s+(?:do|would|should)\s+(?:you|i|we)\s+say\b|"
        r"^how\s+(?:do|does|would|should)\s+"
        r"(?:(?:you|i|we)\s+)?pronounce\b|"
        r"^how\s+is\b.{1,120}\bpronounced\b|"
        r"^what\s+does\b.{1,120}\bmean\b|"
        r"^tell\s+me\s+what\b.{1,120}\bmeans\b|"
        r"^(?:please\s+)?tell\s+me\s+the\s+"
        r"(?:meaning|translation|definition)\s+of\b|"
        r"^what(?:'s|\s+is)\s+.{1,120}\s+in\s+"
        r"(?:English|Chinese|Japanese|Korean|French|German|Spanish)\s*[?？]?$|"
        r"^(?:what\s+is\s+the\s+)?"
        r"(?:meaning|translation|definition)\s+of\b",
        re.IGNORECASE | re.DOTALL,
    ),
    re.compile(
        r"^what(?:'s|\s+is)\s+the\s+(?:meaning|translation|definition)\s+of\b|"
        r"^how\s+(?:do|should|would|can|could)\s+(?:you|i|we)\s+translate\b",
        re.IGNORECASE,
    ),
)

_CONVERSATION_REQUEST_PATTERNS = (
    re.compile(
        r"^(?:請回答|幫我回答|請告訴我|可以告訴我|能告訴我|"
        r"請解釋|幫我解釋|請分析|幫我分析|請比較|幫我比較|"
        r"給我建議)",
        re.IGNORECASE,
    ),
    re.compile(
        r"^(?:please\s+)?(?:answer|explain|compare|analy[sz]e|"
        r"tell\s+me|show\s+me|help\s+me\s+(?:understand|decide))\b",
        re.IGNORECASE,
    ),
)

_UNSUPPORTED_FAST_TRANSLATION_TARGET_PATTERN = re.compile(
    r"(?:翻譯|翻成|譯成).{0,24}(?:日文|日語|韓文|韓語|法文|法語|德文|德語|"
    r"西班牙文|西班牙語)|\btranslate\b.{0,40}\b(?:Japanese|Korean|French|"
    r"German|Spanish)\b",
    re.IGNORECASE | re.DOTALL,
)

_SEMANTIC_LANGUAGE_QUESTION_PATTERN = re.compile(
    r"(?:是什麼意思|意思是什麼|怎麼念|如何發音|怎麼發音|音標是什麼|"
    r"的?(?:定義|意思)(?:是|為)什麼)|"
    r"^(?:what\s+does\b.{1,120}\bmean|tell\s+me\s+what\b.{1,120}\bmeans|"
    r"what(?:'s|\s+is)\s+the\s+(?:meaning|definition)\s+of|"
    r"(?:can|could|would|will)\s+you\s+(?:please\s+)?(?:pronounce|define)|"
    r"how\s+(?:do|does|would|should|can|could)\s+"
    r"(?:(?:you|i|we)\s+)?pronounce|how\s+is\b.{1,120}\bpronounced)",
    re.IGNORECASE | re.DOTALL,
)

_MULTILINE_TRANSLATION_COMMAND_PATTERN = re.compile(
    r"^(?:(?:(?:翻譯|請(?:你|妳)?(?:幫我)?翻譯|幫我翻譯|"
    r"麻煩(?:你|妳)?翻譯)"
    r"(?:成[^\s:：，,。！？?]{1,20})?|"
    r"(?:請(?:你|妳)?(?:幫我)?|幫我|麻煩(?:你|妳)?)?"
    r"(?:翻成|譯成)[^\s:：，,。！？?]{1,20})"
    r"(?=[:：\s]|以下|這|$)|"
    r"(?:please\s+)?translate|(?:can|could|would|will)\s+you\s+"
    r"(?:please\s+)?translate\b)",
    re.IGNORECASE,
)

_SINGLE_LINE_QUESTION_PATTERNS = (
    re.compile(
        r"^(?:請問|想請問|我想問|我想知道|你覺得|你認為|你知道|"
        r"為什麼|怎麼辦|怎麼做|如何|什麼是|誰是|"
        r"哪(?:個|些|裡|一)|何時|何處|多久|多少|"
        r"幾(?:點|個|歲|次)|是否|有沒有|能否|可否|是不是)",
        re.IGNORECASE,
    ),
    re.compile(
        r"^(?:why|who|where|when|which)\s+"
        r"(?:is|are|am|was|were|do|does|did|have|has|had|"
        r"can|could|will|would|should|may|might|must)\b|"
        r"^(?:what\s+(?:is|are|was|were|should|can)|"
        r"how\s+(?:do|does|did|can|could|should|would|is|are)|"
        r"do\s+you|can\s+you|could\s+you|should\s+i|would\s+you)\b",
        re.IGNORECASE,
    ),
    re.compile(
        r"^who\s+(?:won|lost|said|made|wrote|owns|runs|created|invented|"
        r"called|sent|took|gave|did|does|can|could|will|would|should)\b|"
        r"^what\s+(?:happened|changed|caused|makes?|made|went|comes?)\b|"
        r"^which\s+.+\s+(?:is|are|was|were|do|does|did|can|could|will|"
        r"would|should)\b|"
        r"^how\s+(?:many|much|long|often|far|old)\b|"
        r"^what\s+(?:time|day|date|year|kind|type|colou?r|size|name)\b|"
        r"^where\s+to\b|^when\s+(?:next|exactly)\b|^why\s+(?:not|so)\b",
        re.IGNORECASE,
    ),
    re.compile(
        r"^(?:is|are|am|was|were|do|does|did|have|has|had|"
        r"can|could|will|would|should|may|might|must)\s+\S+",
        re.IGNORECASE,
    ),
)


def _looks_like_direct_question(value: str) -> bool:
    """Return whether one line is unambiguously asking Grace a question."""
    text = str(value or "").strip()
    if not text:
        return False
    words = re.findall(r"[A-Za-z]+", text.rstrip("?？").strip())
    ...
    if looks_like_title:
        return False
    if text.endswith(("?", "？")) and not looks_like_title:
        return True
    if any(pattern.search(text) for pattern in _SINGLE_LINE_QUESTION_PATTERNS):
        return True
    return bool(re.search(
        r"(?:嗎|呢|是不是|有沒有|是什麼|做什麼|為什麼|怎麼樣|"
        r"怎麼辦|哪裡|哪個|誰|幾點|幾號|多少錢)\s*$",
        text,
    ))


_CONVERSATION_CORRECTION_PATTERN = re.compile(
    r"^(?:(?:我)?(?:剛剛|前面|上一(?:句|則|個)|這(?:句|則|個)|"
    r"那(?:句|則|個)).{0,30}(?:不是|並非|不要|不用)"
    r".{0,12}(?:翻譯|翻成|譯成)|"
    r"(?:剛剛|前面|上一(?:句|則|個)).{0,20}(?:是|在問).{0,10}"
    r"(?:問題|你|妳)|"
    r"(?:這|那|剛剛).{0,12}(?:是個問題|是問題)|"
    r"(?:請|要|直接)(?:回答|回覆)(?:我|問題)|"
    r"(?:my\s+)?(?:previous|last)\s+(?:turn|message)\s+was\s+"
    r"a\s+question.{0,30}(?:not\s+(?:a\s+)?translation)?)",
    re.IGNORECASE,
)

_CONTINUATION_STATEMENT_PATTERN = re.compile(
    r"^(?:我(?:是想|想要|要的是|的意思|指的是|剛剛|其實)|"
    r"這(?:個|樣|裡|次|部分)|那(?:個|樣|裡|部分)|"
    r"其實|所以|因此|因為|不過|但是|而且|另外|還有|"
    r"對[，,\s]|不是[，,\s]|也就是|換句話說|"
    r"再(?:幫我)?(?:短|長|自然|口語|正式|有趣|精簡|詳細|清楚|簡單|"
    r"直接|活潑|專業|改|換|調整|試|給)|"
    r"actually\b|i\s+mean\b|what\s+i\s+mean\b|"
    r"so\b|because\b|but\b|also\b|"
    r"(?:make|keep|change|try|give)\s+(?:it|that|this)\b|"
    r"(?:shorter|longer|more|less)\b)",
    re.IGNORECASE,
)

_LEADING_DISCOURSE_MARKERS_PATTERN = re.compile(
    r"^(?:(?:(?:我其實|其實|另外|所以|因此|因為|不過|但是|而且|還有|"
    r"也就是|換句話說)\s*[,，:：]?\s*)|"
    r"(?:(?:對|不是)\s*[,，:：]\s*)|"
    r"(?:(?:actually|i\s+mean|what\s+i\s+mean|also|so|because|but)"
    r"\b\s*[,，:：]?\s*))+",
    re.IGNORECASE,
)


def _history_user_text(value: Any) -> str:
    if isinstance(value, dict):
        value = value.get("preview") or value.get("content") or ""
    text = str(value or "").strip()
    return re.sub(r"^\[[^\]\n]{1,80}\]\s*", "", text).strip()


def _response_profile_current_intent(
    profile: Dict[str, Any],
    text: Any,
) -> str:
    """Classify one turn without consulting prior Topic turns."""
    cleaned = str(text or "").strip()
    if not cleaned:
        return "conversation"
    if any(pattern.search(cleaned) for pattern in _TRANSLATION_INTENT_PATTERNS):
        explicit_translation = True
    else:
        explicit_translation = False

    # With no explicit request syntax, use the final effective line to
    # distinguish a pasted passage whose first line happens to be a question
    # from context followed by a direct question to Grace.
    if "\n" in cleaned:
        ...
        if (
            ...
            and lines
            and any(
                _MULTILINE_TRANSLATION_COMMAND_PATTERN.search(line)
                for line in lines
            )
        ):
            return "translation"
        if any(
            ...
            for pattern in _CONVERSATION_REQUEST_PATTERNS
        ):
            return "conversation"
        return (
            "conversation"
            if lines and _looks_like_direct_question(lines[-1])
            else "translation"
        )
    if explicit_translation:
        return "translation"
    if any(pattern.search(cleaned) for pattern in _CONVERSATION_REQUEST_PATTERNS):
        return "conversation"
...
def _has_explicit_translation_intent(text: Any) -> bool:
    cleaned = str(text or "").strip()
    return bool(cleaned) and any(
        pattern.search(cleaned)
        for pattern in _TRANSLATION_INTENT_PATTERNS
    )


def _has_authoritative_translation_request(text: Any) -> bool:
    cleaned = str(text or "").strip()
    if not cleaned:
        return False
    without_discourse_marker = _LEADING_DISCOURSE_MARKERS_PATTERN.sub(
        "",
        cleaned,
    )
    if "\n" not in without_discourse_marker:
        return _has_explicit_translation_intent(without_discourse_marker)
    return any(
        _MULTILINE_TRANSLATION_COMMAND_PATTERN.search(line.strip())
        for line in without_discourse_marker.splitlines()
        if line.strip()
    )


def response_profile_intent(
    profile: Dict[str, Any],
    text: Any,
    *,
    recent_user_messages: Any = None,
) -> str:
    """Choose translation or Grace conversation with bounded Topic context.

    Explicit translation/meaning requests win over question syntax.  Clear
    information-seeking questions go to the conversational agent.  Ambiguous
    standalone content keeps the Translator Topic's translation-first default,
    except a continuation statement inherits the immediately preceding user
    turn's intent from the same session/Topic.
    """
    if profile.get("intent_gate") != "translation_or_answer":
        return "translation"
    cleaned = str(text or "").strip()
    recent_items = list(recent_user_messages or ())
    if (
        _UNSUPPORTED_FAST_TRANSLATION_TARGET_PATTERN.search(cleaned)
        or _SEMANTIC_LANGUAGE_QUESTION_PATTERN.search(cleaned)
    ):
        return "conversation"
    # A clear current-turn translation request remains authoritative even when
    # it begins with continuation language such as "Actually" or contains
    # source text that resembles an intent correction.
    if _has_authoritative_translation_request(cleaned):
        return "translation"
    if recent_items and _CONVERSATION_CORRECTION_PATTERN.search(cleaned):
        return "conversation"
    current_intent = _response_profile_current_intent(profile, cleaned)
    if current_intent == "conversation":
        return current_intent
    if not _CONTINUATION_STATEMENT_PATTERN.search(cleaned):
        return current_intent
    for index, item in enumerate(recent_items):
        previous_text = _history_user_text(item)
        if not previous_text:
            continue
        if (
            _CONVERSATION_CORRECTION_PATTERN.search(previous_text)
            and index + 1 < len(recent_items)
        ):
            return "conversation"
        previous_intent = _response_profile_current_intent(
            profile,
            previous_text,
        )
        if (
            previous_intent == "translation"
            and _CONTINUATION_STATEMENT_PATTERN.search(previous_text)
            and not _has_authoritative_translation_request(previous_text)
        ):
            continue
