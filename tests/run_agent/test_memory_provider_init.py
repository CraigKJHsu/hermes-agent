...
def test_aiagent_disables_memory_provider_after_init_timeout(monkeypatch):
    ...
    cfg = {"memory": {"provider": "recording"}, "agent": {}}
    monkeypatch.setenv("HERMES_MEMORY_PROVIDER_INIT_TIMEOUT", "0.01")
    ...
    assert agent._memory_provider_init_timed_out is True
    assert agent._memory_manager is None
    release.set()
