...
def test_session_store_loads_bounded_recent_user_texts():
    store = object.__new__(SessionStore)
    calls = []

    class _DB:
        def list_recent_user_messages(self, session_id, limit):
    ...
    assert store.load_recent_user_texts("session-1", limit=99) == [
        "newest",
        "older",
    ]
    assert calls == [("session-1", 5)]
...
def test_prepare_direct_translation_input_and_target():
    ...
    assert prepare_direct_translation_input(
        "Please translate this to Chinese: hello",
    ) == ("hello", "zh-TW")
    assert prepare_direct_translation_input(
        "Hello，請翻譯成繁體中文",
    ) == ("Hello", "zh-TW")
...
def test_start_response_fast_lane_routes_clear_question_to_grace():
    runner = object.__new__(GatewayRunner)
    event = MessageEvent(
        text="為什麼天空是藍色的？",
        source=_source(),
...
@pytest.mark.asyncio
async def test_start_response_fast_lane_keeps_explicit_translation_question():
    from gateway.response_profiles import _FAST_LANE_HANDLERS
    ...
    mocked = AsyncMock(return_value="Are you free tomorrow?")
    with patch.dict(_FAST_LANE_HANDLERS["translation"], {"run": mocked}):
        job = runner._start_response_fast_lane(event)
        assert job is not None
        assert await job["task"] == "Are you free tomorrow?"


def test_start_response_fast_lane_uses_recent_question_for_continuation():
    runner = object.__new__(GatewayRunner)
    event = MessageEvent(
        text="我是想要有一點有趣的title來經營這個粉絲專頁的媒體",
        source=_source(),
        response_profile={
            "strategy": "fast_then_default",
            "intent_gate": "translation_or_answer",
    ...
    assert runner._start_response_fast_lane(
        event,
