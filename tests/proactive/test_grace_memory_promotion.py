...
def test_topic_archive_serializes_concurrent_promotions(tmp_path, monkeypatch):
    home = tmp_path / ".hermes"
    monkeypatch.setenv("HERMES_HOME", str(home))
    errors = []

    def archive(promotion_id, entry):
        try:
            promotion._archive_topic(
                NAMESPACE,
                [entry],
                review_task_id=f"review-{promotion_id}",
    ...
    threads = [
        threading.Thread(target=archive, args=("one", "entry one")),
        threading.Thread(target=archive, args=("two", "entry two")),
    ]
    ...
    assert errors == []
    _markdown, state_path = promotion._topic_paths(NAMESPACE)
    state = json.loads(state_path.read_text(encoding="utf-8"))
    assert set(state["entries"]) == {"entry one", "entry two"}
    assert {source["promotion_id"] for source in state["sources"]} == {
        "one", "two",
    }
